...
function getPlayableCards(cards: Card[], gameState: GameState): Card[] {
  ...
  if (!effectiveTopCard) return cards;

  return cards.filter(card => canPlayCard(card, effectiveTopCard, isThreeActive));
}

// Group cards by their value
function groupCardsByValue(cards: Card[]): Map<string | number, Card[]> {
  const groups = new Map<string | number, Card[]>();
...
function findBestPlay(playableCards: Card[]): Card[] {
  ...
  const powerCard = playableCards.find(card => 
    card.value === 10 || card.value === 'joker'
  );
  if (powerCard) {
    return [powerCard];
  }
  ...
  const specialCard = playableCards.find(card => 
    [2, 3, 7].includes(card.value as number)
  );
  if (specialCard) {
    return [specialCard];
  }
  ...
  const regularCards = playableCards.filter(card => 
