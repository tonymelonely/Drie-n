...
export function getGameHint(gameState: GameState): GameHint | null {
  ...
  if (gameState.gamePhase === 'selectingCards') {
    const specialCards = currentPlayer.hand.filter(card => 
      [2, 3, 10].includes(card.value as number) || card.value === 'joker'
    );
