...
export function findLastNonThreeCard(pile) {
...
export function findLastEffectiveCard(pile) {
    ...
    for (let i = pile.length - 1; i >= 0; i--) {
        ...
        if (lastJokerIndex !== -1 && lastThreeIndex !== -1)
            break;
    }
...
export function countConsecutiveSameValue(pile) {
    if (pile.length === 0)
        return { count: 0, value: 2 };
    let count = 1;
...
export function canPlayCard(card, topCard, isThreeActive = false) {
    // Can always play if there's no top card
    if (!topCard)
        return true;
    // Special cards can always be played
    if (card.value === 'joker' || card.value === 2 || card.value === 10)
        return true;
...
export function canPlayFromFaceUpCards(player) {
    return player.hand.length === 0;
}
export function canPlayFromFaceDownCards(player) {
    return player.hand.length === 0 && player.faceUpCards.length === 0;
}
export function getEffectiveTopCard(pile) {
    if (pile.length === 0) {
        return {
            card: undefined,
            isThreeActive: false,
...
export function shouldKeepTurn(cards, pile) {
...
export function drawCardsFromDeck(gameState, playerId) {
...
export function playCards(gameState, playerId, cards) {
    const player = gameState.players.find(p => p.id === playerId);
    if (!player)
        return gameState;
    ...
    if (canPlayFromFaceDownCards(player)) {
        updatedPlayer.faceDownCards = player.faceDownCards.filter(c => !cards.some(played => played.id === c.id));
    }
    else if (canPlayFromFaceUpCards(player)) {
        updatedPlayer.faceUpCards = player.faceUpCards.filter(c => !cards.some(played => played.id === c.id));
    }
    else {
        updatedPlayer.hand = sortCards(player.hand.filter(c => !cards.some(played => played.id === c.id)));
    }
...
export function handleFaceDownGuess(gameState, playerId, guessedCard) {
    const player = gameState.players.find(p => p.id === playerId);
    if (!player)
        return gameState;
