...
export function CardInfo({ card }: CardInfoProps) {
  ...
  return (
    <div className="absolute -top-2 -right-2 group">
      <button className="w-5 h-5 rounded-full bg-blue-100 flex items-center justify-center">
        <HelpCircle className="w-3 h-3 text-blue-600" />
      </button>
      
      <div className="hidden group-hover:block absolute z-50 right-0 mt-2 w-48 bg-white rounded-lg shadow-xl p-3 text-sm">
        <div className="font-semibold text-gray-800 mb-1">
          {card.value === 'joker' ? 'Joker' : `${card.value} of ${card.suit}`}
        </div>
        ...
        {isSpecialCard && (
          <div className="mt-1 text-xs text-blue-600 font-medium">
            Special card - use strategically!
          </div>
        )}
      </div>
    </div>
